...
void* fs_init(struct fuse_conn_info *conn)
{
	...
	size_t inode_map_size = sb.inode_map_sz;
	inode_map = malloc(inode_map_size*FS_BLOCK_SIZE);
	if(disk->ops->read(disk, inode_map_base, inode_map_size, inode_map)){
		exit(EXIT_FAILURE);
	}

	// read block map
	block_map_base = inode_map_base + inode_map_size;
	size_t block_map_size = sb.block_map_sz;
	block_map = malloc(block_map_size*FS_BLOCK_SIZE);
	if(disk->ops->read(disk, block_map_base, block_map_size, block_map)){
		exit(EXIT_FAILURE);
	}

	/* The inode data is in the next set of blocks */
	inode_base = block_map_base + block_map_size;
	size_t inode_region_size = sb.inode_region_sz;
	n_inodes = inode_region_size * INODES_PER_BLK;
	inodes = malloc(inode_region_size*FS_BLOCK_SIZE);
	if(disk->ops->read(disk, inode_base, inode_region_size, inodes)){
		exit(EXIT_FAILURE);
	}
...
{
    // Ensure that mode represents a directory and path is not root
    mode |= S_IFDIR;
    if (!S_ISDIR(mode) || strcmp(path, "/") == 0)
        return -EINVAL;

    // Get inode index for the given path
    char *_path = strdup(path);
    char name[FS_FILENAME_SIZE];
    int inode_idx = translate(_path);
    int parent_inode_idx = translate_1(_path, name);

    // Check if directory already exists or parent directory is invalid
    if (inode_idx >= 0)
        return -EEXIST;
    if (parent_inode_idx < 0)
        return parent_inode_idx;

    // Retrieve parent directory's inode
    struct fs_inode *parent_inode = &inodes[parent_inode_idx];
    if (!S_ISDIR(parent_inode->mode))
        return -ENOTDIR;

    // Read directory entries from parent directory
    struct fs_dirent entries[DIRENTS_PER_BLK];
    memset(entries, 0, DIRENTS_PER_BLK * sizeof(struct fs_dirent));
    if (disk->ops->read(disk, parent_inode->direct[0], 1, entries) < 0)
		return EXIT_FAILURE;

    // Set attributes for the new directory and update directory entries
...
{
    // Check if attempting to remove root directory
    if (strcmp(path, "/") == 0)
        return -EINVAL;

    // Get inode indices for the directory and its parent
    char *_path = strdup(path);
    char name[FS_FILENAME_SIZE];
    int inode_idx = translate(_path);
    int parent_inode_idx = translate_1(_path, name);

    // Retrieve directory and parent directory inodes
    struct fs_inode *inode = &inodes[inode_idx];
    struct fs_inode *parent_inode = &inodes[parent_inode_idx];

    // Check if directory and parent directory exist
    if (inode_idx < 0 || parent_inode_idx < 0)
        return -ENOENT;

    // Ensure that directory inode represents a directory
    if (!S_ISDIR(inode->mode) || !S_ISDIR(parent_inode->mode))
        return -ENOTDIR;

    // Read directory entries to check if directory is empty
    struct fs_dirent entries[DIRENTS_PER_BLK];
    memset(entries, 0, DIRENTS_PER_BLK * sizeof(struct fs_dirent));
    ...
    // Find and clear directory entry in parent directory
    for (int i = 0; i < DIRENTS_PER_BLK; i++) {
        if (entries[i].valid && strcmp(entries[i].name, name) == 0) {
            memset(&entries[i], 0, sizeof(struct fs_dirent));
        }
    ...
    // Return blocks and clear inode for the directory
    return_blk(inode->direct[0]);
    memset(inode, 0, sizeof(struct fs_inode));
    return_inode(inode_idx);

    // Update inode and block metadata
    update_inode(inode_idx);
    update_blk();
...
{
    // Get inode indices for the file and its parent directory
    char* _path = strdup(path);
    char name[FS_FILENAME_SIZE];
    int inode_idx = translate(_path);
    int parent_inode_idx = translate_1(_path, name);

    // Check if file and parent directory exist
    if (inode_idx < 0 || parent_inode_idx < 0)
        return -ENOENT;

    // Retrieve file and parent directory inodes
    struct fs_inode *inode = &inodes[inode_idx];
    struct fs_inode *parent_inode = &inodes[parent_inode_idx];

    // Ensure that parent inode represents a directory
    if (!S_ISDIR(parent_inode->mode))
        return -ENOTDIR;

    // Update modification time of the file
...
static void fs_read_blk(int blk_num, char *buf, size_t len, size_t offset) {
    // Read data from the block and store it in the buffer
    char data[BLOCK_SIZE];
	memset(data, 0, BLOCK_SIZE);
    if (disk->ops->read(disk, blk_num, 1, data) < 0)
        exit(1);
...
static int fs_read(const char *path, char *buf, size_t len, off_t offset,
    ...
    // Adjust length if reading beyond EOF
    if (offset + len > inode->size)
        len = inode->size - offset;

    // Read data from the file blocks
    size_t len_to_read = len;
